...
__led_state_enum_to_string(const struct locl_led *led_,
                           enum ovsrec_led_state state);
int
__led_state_string_to_enum(const struct locl_led *led_, const char *value,
                           enum ovsrec_led_state *state);
                ...
                enum ovsrec_led_state *state)
{
    ...
    rc = __led_state_string_to_enum(led_, value, state);
                ...
                enum ovsrec_led_state state)
{
    struct sysfs_led *led = sysfs_led_cast(led_);
    int rc = 0;
    const char *value = __led_state_enum_to_string(led_, state);

    if (NULL == value) {
        VLOG_WARN("Failed to set unsupported sysfs led state %d for "
    ...
    CHECK_RC(rc, "Led sysfs state set %s for %s", value, led->up.name);
                           ...
                           enum ovsrec_led_state state)
{
    switch (state) {
        case LED_STATE_FLASHING:
            return led_->type->settings.flashing;
        case LED_STATE_ON:
            return led_->type->settings.on;
        case LED_STATE_OFF:
            return led_->type->settings.off;
        default:
            VLOG_WARN("Invalid sysfs state %d for subsystem %s, LED %s",
...
__led_state_string_to_enum(const struct locl_led *led_, const char *value,
                           enum ovsrec_led_state *state)
{
    const size_t len = NAME_MAX;

    if (!strncmp(value, led_->type->settings.on, len)) {
        *state = LED_STATE_ON;
    } else if (!strncmp(value, led_->type->settings.off, len)) {
        *state = LED_STATE_OFF;
    } else if (!strncmp(value, led_->type->settings.flashing, len)) {
