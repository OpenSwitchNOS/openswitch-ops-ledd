...
YamlLedType *
ledd_get_led_type(struct ledd_locl_subsystem *subsys, char *value)
{
...
/************************************************************************//**
 * Function that will remove the internal entry in the ledd_locl_subsystem hash
...
ledd_remove_unmarked_subsystems(void)
{
    ...
    SHASH_FOR_EACH_SAFE(node, next, &subsystem_data) {
        struct ledd_locl_subsystem *subsystem = node->data;
 ...
 ***************************************************************************/
bool
ledd_write_led(struct ledd_locl_subsystem *subsys, struct locl_led *led)
{
    ...
    switch (type_value) {
        case LED_LOC:
            rc = led->class->ledd_led_state_set(led, led->state);
            if (rc != 0) {
                          ...
                          const char *argv[] OVS_UNUSED, void *aux OVS_UNUSED)
{
    ...
    SHASH_FOR_EACH(snode, &subsystem_data) {
        struct ledd_locl_subsystem *subsystem = (struct ledd_locl_subsystem *)snode->data;
...
ledd_unmark_subsystems(void)
{
    ...
    SHASH_FOR_EACH(node, &subsystem_data) {
        struct ledd_locl_subsystem *subsystem = (struct ledd_locl_subsystem *)node->data;
 ...
 ***************************************************************************/
void
process_changes_in_subsys(struct ledd_locl_subsystem *subsys)
{
                            ...
                            struct ovsdb_idl_txn *txn)
{
    ...
    struct ledd_locl_subsystem *lsubsys = NULL;
    int rc = 0;
    int idx = 0;
    int led_count = 0;
    int type_count = 0;
    ...
    subsystem_class = ledd_subsystem_class_get(LEDD_PLATFORM_TYPE_STR);
    led_class = ledd_led_class_get(LEDD_PLATFORM_TYPE_STR);
    ...
    /* Add the types to the ledd_locl_subsystem structure */
    ...
    /* Update the state of the ledd_locl_subsystem structure */
    lsubsys->marked = true;
...
ledd_reconfigure(void)
{
    ...
    OVSREC_SUBSYSTEM_FOR_EACH(ovs_sub, idl) {
        struct ledd_locl_subsystem *subsystem;
